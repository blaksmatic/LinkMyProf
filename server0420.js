        ...
        Professor.find({},function(err, curProfessor){
            for(i =0; i < curProfessor.length; i++) {
                ...
                if (contains(currLoca, curProfessor[i].loca)){
                    User.update({username: req.params.username}, {$push: {profRecIndex: {profid: curProfessor[i].id, ind: 0.25+0.1*Math.random(),field:'area'}}},
                        function (err, User) {});
                }
                else {
                    User.update({username: req.params.username}, {$push: {profRecIndex: {profid: curProfessor[i].id, ind: 0.0, field:'area'}}},
                        function (err, User) {});
                }
            }
        ...
        User.find({username: {$ne: req.params.username}}, function (err, refUser) {
            ...
            User.find({username: favorlist[0][0]}, function (err, refUser2) {
                ...
                for (i = 0; i < simiidlist.length; i++) {
                    User.update({username: req.params.username}, {$push: {profRecIndex: {profid: simiidlist[i], ind: 0.1+0.1*Math.random(),field:'popularity'}}},
                        function (err, User) {});
                }
            })
        })
